...
from tgen.tgen.seq2seq import cut_batch_into_steps, grouper
...
class Seq2SeqGen(Seq2SeqBase, TFModel):
    ...
    def _load_valid_data(self, valid_data_paths):
        ...
        valid_trees = read_trees_or_tokens(valid_trees_file, self.mode, self.language, self.ref_selectors,
                                           ref_mode=True)
    ...
    def _cut_valid_data(self):
        ...
        # we have multiple references
            ...
            if refs_stored == 'serial':
                ...
                print(len(tree_chunks[0]), chunk_len, self.validation_size)
                print(list(range(chunk_len - self.validation_size, chunk_len)))
                print([len(chunk) for chunk in tree_chunks])
    ...
    def _init_neural_network(self):
        ...
        model_vars = [var for var in tf.global_variables() if
                      not (var.name.startswith('rerank-') or var.name.startswith('formselect-'))]
