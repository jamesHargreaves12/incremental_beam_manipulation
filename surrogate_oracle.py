...
if os.path.exists(cfg["surrogate_train_data_path"]):
    print("Loading Training data")
    surrogte_train_dict = msgpack.load(open(cfg["surrogate_train_data_path"], 'rb+'),use_list=False, strict_map_key=False)
else:
    print("Creating Training data")
    surrogte_train_dict = {}
    scorer_func = get_greedy_decode_score_func(models, final_scorer=final_scorer, max_length_out=text_embedder.length, save_scores=surrogte_train_dict)

    preds = run_beam_search_with_rescorer(scorer_func, models, das[:500], 3, cfg['only_rerank_final'],
                                          cfg.get('beam_save_path', None))
...
reranker.train(text_seqs[:-valid_size], da_seqs[:-valid_size], scores[:-valid_size], cfg["trainable_reranker_epoch"], text_seqs[-valid_size:], da_seqs[-valid_size:], scores[-valid_size:])
